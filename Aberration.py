...
def MacroLens_Lens12345(RAD):
    ...
    for i in raySPoint0:
        ...
        directionVector0 = rayStartV + raySPoint0 + np.array([0*100, 0*100, 10000*np.sin(RAD)*100])
        T = VF.rayTraceDecideT_Lens1L(raySPoint0, directionVector0)  # 交点のための係数
        rayEPoint0 = raySPoint0 + T*directionVector0 + np.array([0, 0, -2.5*np.sin(RAD)])  # 入射光の終点
...
def Aberration_Vertical(RAD):
    ...
    pointsY = np.array([centerY]*len(pointsZ))
    ...
    for i in raySPoint0:
        ...
        RedAberration_z.append(rayEPoint_Last[2])
        ...
        BlueAberration_z.append(rayEPoint_Last[2])

    count = 0
    for i in pointsZ:
        ax.scatter(0, i, RedAberration_z[count]-RedAberration_z[30], color='r', s=1)
        ax.scatter(0, i, BlueAberration_z[count]-BlueAberration_z[30], color='b', s=1)
        count += 1


    if RAD==0.00:
        ax.set_title('Lateral Aberration', pad=-7)
    ax.set_xlim(-LX/2, LX/2)
    ax.set_ylim(-LY, LY)
    ax.set_zlim(-1/4, 1/4)
    #ax.set_xlabel('x')
    ax.axes.xaxis.set_ticks([])
    ax.set_ylabel('Pupil Points_z / cm')
    ax.set_zlabel('dz', labelpad=-1)
    ax.view_init(elev=0, azim=0)


# 横収差 焦点前後
def Aberration_Surface(RAD):
    def T_FocusGraph(startV, directionV):
        T = -startV[2]/directionV[2]
        return T

    # スクリーンとの交点を持つときの係数Ｔを求める関数(デフォーカス対応)
    def rayTraceDecideT_Screen_dfocus(startV, directionV, dfocus):
        T = (screenV[0]+dfocus-startV[0])/directionV[0]
        return T
    ...
    for i in raySPoint0:
        for j in np.arange(-0.2, 0.2, 0.05):
            raySPoint0 = i
            #directionVector0 = np.array([1, 0, 0])  # 入射光の方向ベクトルを設定
            directionVector0 = rayStartV + raySPoint0 + np.array([0*100, 0*100, 10000*np.sin(RAD)*100])
            T = VF.rayTraceDecideT_Lens1L(raySPoint0, directionVector0)  # 交点のための係数
            ...
            T = rayTraceDecideT_Screen_dfocus(raySPoint_Lens5R, refractionV_Lens5R, j)
            rayEPoint_Last = raySPoint_Lens5R + T*refractionV_Lens5R
            #VF.plotLineRed(raySPoint_Lens5R, rayEPoint_Last)

            RedAberration_x.append(rayEPoint_Last[0])
            RedAberration_y.append(i[2])
            RedAberration_z.append(rayEPoint_Last[2])
            ...
            BlueAberration_x.append(rayEPoint_Last[0])
            BlueAberration_y.append(i[2])
            BlueAberration_z.append(rayEPoint_Last[2])

    for i in range(len(RedAberration_x)):
        ax.scatter(RedAberration_x[i], RedAberration_y[i], RedAberration_z[i]-RedAberration_z[244], color='r', s=1)
        ax.scatter(BlueAberration_x[i], BlueAberration_y[i], BlueAberration_z[i]-BlueAberration_z[244], color='b', s=1)


    if RAD==0.00:
        ax.set_title('Lateral Aberration_Surface', pad=-7)
    ax.set_xlim(-LX/4+10.86973606, LX/4+10.86973606)
    ax.set_ylim(-LY, LY)
    ax.set_zlim(-1/4, 1/4)
    ax.set_xlabel('d Focus Points')
    ax.set_ylabel('Pupil Points_z / cm')
    ax.set_zlabel('dz', labelpad=-1)
